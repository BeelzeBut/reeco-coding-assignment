...
**Two URLs, two wire formats — provisional, see `docs/pending-decisions.md` §1.** The bulk endpoint is exposed at both `POST /api/orders/bulk-action` (camelCase body/response, matches README §1.5 and `bulk-operations.test.ts`) and `POST /api/orders/bulk-actions` (snake_case body/response, matches `concurrency.test.ts`). Both routes call the same `BulkService`. This dual-shape exists because the two test files disagree on URL and casing; both are authoritative. Pending PO review to collapse to one style.
