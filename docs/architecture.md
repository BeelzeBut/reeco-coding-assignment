...
**Wire format.** The bulk endpoint is the only API surface that uses **camelCase** (`orderIds`, `jobId`) — every other endpoint uses snake_case. This matches the README §1.5 example exactly. DTOs use `[JsonPropertyName(...)]` to opt out of the global snake-case naming policy.
