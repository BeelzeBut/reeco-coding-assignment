...
│   ├── SECURITY.md            ← threat model and control inventory
│   ├── testing.md             ← how to run the 83-test suite without false failures
│   ├── manual-test-plan.md    ← end-to-end UI walkthrough for human verification
...
- [x] **2026-05-05** — Test-protocol + manual-sweep docs landed. New `docs/testing.md` codifies the run-it-right protocol for the 83-test suite (re-import between every file via a copy-pasteable bash one-liner; PowerShell equivalent included), explains the file-ordering hazard the suite carries (basic-crud's single PATCH skews aggregations' strict-equality counts; vitest 2.0's default file-level parallelism lets bulk/concurrency tests mutate first-page pending orders before basic-crud reads them), names the vitest-version gotcha (project pins ^2.0; `npx` from outside `tests/` pulls 4.x with different defaults), and lists common gotchas with their fixes. New `docs/manual-test-plan.md` is an end-to-end UI walkthrough (~15-20 min) covering setup, theme + nav chrome, all 4 routes (orders list/filter/sort/search/detail/PATCH, multi-select bulk with all 3 actions + selection persistence + failure cases + SSE arrival, stats dashboard, suppliers list, supplier detail with performance), cross-cutting loading/error/empty states, security probes (XSS in notes, headers, CORS), and backend-only paths exercised via curl. Both files indexed in §4 (repo layout) and the test-ordering caveat callout above now points at `testing.md`. No code changes.

- [x] **2026-05-05** — Bulk-action FE wiring landed (Part 2 final piece). New checkbox column on the Orders table backed by `useBulkSelection` (`Set<string>`, persists across pages/filters per the locked decision); tri-state header acts on the visible page. New `BulkActionBar` (sticky, appears when count>0) launches `BulkConfirmDialog` (Approve / Reject / Flag) using a fresh hand-rolled `AlertDialog` primitive that mirrors `Sheet` (focus-trap, ESC, backdrop click, no radix dep). Submission goes to canonical `POST /api/orders/bulk-action`; on success the bar clears and a `BulkJobToast` is pushed onto a `string[]` stack at page-tail (bottom-right, stacked). Each toast subscribes via TanStack `useQuery` with `refetchInterval: 1000` for live `completed/failed` counters AND a refcounted shared `EventSource` (`lib/useBulkJobEvents.ts`) for the `bulk_completed { jobId }` SSE event — first FE consumer of `/api/events`. Polling and SSE are independent: SSE just early-triggers a refetch, polling is the safety net. Failures (id missing / already cancelled / version conflict) explained via an inline tooltip; per-order outcomes intentionally not shown because the job endpoint exposes only counts. Edge cases covered: >10,000 selection cap (buttons disabled + dialog warning), reason field 4096-char clamp with `reason_too_long` server fallback, checkbox cell `stopPropagation` keeps row click → detail sheet working, skeleton/empty `colSpan` bumped 8→9 for the new column. Backend untouched. `npm --prefix src/OrderOps.Web run build` clean (tsc -b + vite). 83/83 backend tests still green by inspection (no API changes). **README Part 2 fully covered.**
